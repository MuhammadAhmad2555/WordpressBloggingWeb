...
error_log("Post format: " . ($post_format ? $post_format : 'standard'));
...
<div class="cs-cover <?php echo esc_attr( $class ); ?> cs-cover-<?php echo esc_attr( $post_format ); ?>">
	...
	<header class="cover-header entry-header <?php echo esc_attr( $color_class ); ?>">
		...
		if ( is_singular() ) {
			...
			if ( 'post' === $post_type ) {
				csco_get_post_meta( array( 'author', 'date', 'shares', 'comments', 'views', 'reading_time' ), false, true, 'post_meta' );
			}
		} else {
			// Post Category.
			if ( 'post' === $post_type ) {
					...
					?>
						<div class="owl-slide cover-item cover-content" data-merge="<?php echo esc_attr( $merge ); ?>">
							<div class="cover-content-wrap">
								<?php
								// Post Meta.
								csco_get_post_meta( array( 'views', 'reading_time' ), true, true, true );
