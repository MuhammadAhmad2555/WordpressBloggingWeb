...
if ( ! function_exists( 'csco_kirki_installer_register' ) ) {
	/**
	 * Registers the section, setting & control for the kirki installer.
	 *
	 * @param object $wp_customize The main customizer object.
	 */
	function csco_kirki_installer_register( $wp_customize ) {
		// Early exit if Kirki exists.
		if ( class_exists( 'Kirki' ) ) {
			return;
		}

		if ( class_exists( 'WP_Customize_Section' ) && ! class_exists( 'CSCO_Kirki_Installer' ) ) {
			/**
			 * Recommend the installation of Kirki using a custom section.
			 *
			 * @see WP_Customize_Section
			 */
			class CSCO_Kirki_Installer extends WP_Customize_Section {
				...
				protected function install_button() {
					...
					<p style="text-align:left;margin-top:0;">
						<?php esc_html_e( 'Please install the Kirki plugin to take full advantage of this theme\s customizer capabilities', 'the-affair' ); ?>
					</p>
					<a class="install-now button-primary button" data-slug="kirki" href="<?php echo esc_url_raw( $this->get_plugin_install_url() ); ?>" aria-label="<?php esc_attr_e( 'Install Kirki Toolkit now', 'the-affair' ); ?>" data-name="Kirki Toolkit">
						<?php esc_html_e( 'Install Now', 'the-affair' ); ?>
					</a>
					<?php
				}
				...
				protected function activate_button() {
					?>
					<p style="text-align:left;margin-top:0;">
						<?php esc_html_e( 'You have installed Kirki. Activate it to take advantage of this theme\'s features in the customizer.', 'the-affair' ); ?>
					</p>
					<a class="activate-now button-primary button" data-slug="kirki" href="<?php echo esc_url_raw( self_admin_url( 'plugins.php' ) ); ?>" aria-label="<?php esc_attr_e( 'Activate Kirki Toolkit now', 'the-affair' ); ?>" data-name="Kirki Toolkit">
						<?php esc_html_e( 'Activate Now', 'the-affair' ); ?>
					</a>
					<?php
				}
				/**
				...
				protected function dismiss_button() {
					...
					$show_confirm = true;
					?>
					<a class="kirki-installer-dismiss button-secondary button" data-slug="kirki" href="#" aria-label="<?php esc_attr_e( 'Don\'t show this again', 'the-affair' ); ?>" data-name="<?php esc_attr_e( 'Dismiss', 'the-affair' ); ?>">
						<?php esc_html_e( 'Don\'t show this again', 'the-affair' ); ?>
					</a>
					...
					jQuery( document ).ready( function() {
						jQuery( '.kirki-installer-dismiss' ).on( 'click', function( event ) {
							...
							<?php if ( $show_confirm ) : ?>
								if ( ! confirm( '<?php esc_html_e( 'Are you sure? Dismissing this message will hide the installation recommendation and you will have to manually install and activate the plugin in the future.', 'the-affair' ); ?>' ) ) {
									return;
								}
							<?php endif; ?>
							jQuery.post( ajaxurl, {
								action: 'csco_kirki_installer_dismiss',
		...
		if ( is_callable( array( 'CSCO_Kirki_Installer', 'is_dismissed' ) ) && CSCO_Kirki_Installer::is_dismissed() ) {
			return;
		}
		$wp_customize->add_section(
			new CSCO_Kirki_Installer(
...
add_action( 'customize_register', 'csco_kirki_installer_register', 999 );

if ( ! function_exists( 'csco_kirki_installer_dismiss' ) ) {
	/**
	 ...
	 */
	function csco_kirki_installer_dismiss() {
	...
	add_action( 'wp_ajax_csco_kirki_installer_dismiss', 'csco_kirki_installer_dismiss' );
}
