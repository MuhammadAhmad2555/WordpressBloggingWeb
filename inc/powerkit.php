...
if ( ! function_exists( 'powerkit_pinit_add_selectors' ) ) {
	/**
	 * PinIt add selectors
	 *
...
if ( ! function_exists( 'csco_powerkit_featured_posts_carousel' ) ) {
	/**
...
if ( ! function_exists( 'csco_powerkit_carousel_template' ) ) {
	/**
			...
			<div class="cs-overlay cs-overlay-ratio cs-bg-dark cs-ratio-portrait">
				...
				<div class="cs-overlay-content">
					...
					if ( function_exists( 'csco_get_post_meta' ) ) {
						csco_get_post_meta( array_diff( $params['post_meta'], array( 'category' ) ), (bool) $params['post_meta_compact'] );
					}
					?>

					<div class="entry-more-button">
						<a class="button" href="<?php echo esc_url( get_permalink() ); ?>">
							<?php echo esc_html( get_theme_mod( 'label_readmore', __( 'View Post', 'the-affair' ) ) ); ?>
						</a>
					</div>
				</div>
