...
function register_csco_demos_control() {
	...
	final class CSCO_Demos_Control extends WP_Customize_Control {
		...
		protected function render_content() {
			?>
				<h3><?php esc_html_e( 'Reset', 'the-affair' ); ?></h3>

				<p><?php esc_html_e( 'Click the button to reset the customization settings for this theme.', 'the-affair' ); ?></p>

				<p>
					<input type="submit" name="csco-demos-reset" class="button-secondary button" value="<?php esc_html_e( 'Reset', 'the-affair' ); ?>">
				...
				<?php if ( isset( $demos['demos'] ) && $demos['demos'] ) { ?>
					<h3><?php esc_html_e( 'Select Demo', 'the-affair' ); ?></h3>
									...
									<div class="theme csco-theme-demo" style="width: 100%;" data-demo="<?php echo esc_attr( $slug ); ?>">
										<?php if ( isset( $settings['preview_image_url'] ) ) { ?>
											<div class="screenshot">
												<img src="<?php echo esc_url( get_template_directory_uri() . $settings['preview_image_url'] ); ?>">
											</div>
										<?php } ?>
										...
										<div class="theme-id-container">
											...
											<div class="theme-actions">
												...
												<a class="button button-primary csco-demo-import" href="#"><?php esc_html_e( 'Activate', 'the-affair' ); ?></a>
											</div>
										</div>
									</div><br>
								<?php
...
class CSCO_Demos_Core {
	...
	public function register( $wp_customize ) {
		...
		$wp_customize->add_section(
			'csco-section', array(
				'title'    => __( 'Theme Demos', 'the-affair' ),
				'priority' => 2,
			)
		);
		...
		<script>
			jQuery(function ($) {
				$('body').on('click', '.csco-demo-import', function (event) {
					...
					var r = confirm( "<?php esc_html_e( 'Warning: Activating a demo will reset all current customizations.', 'the-affair' ); ?>" );
						...
						try {
							...
							if( typeof info.success != 'undefined' && info.success == true ){
								...
								if( typeof info.data != 'undefined' ){
									...
									alert( '<?php esc_html_e( 'Server error!', 'the-affair' ); ?>' );
								}
							}
						} catch (e) {
							alert( response );
		...
		<script>
			jQuery(function ($) {
				$('body').on('click', 'input[name="csco-demos-reset"]', function (event) {
					...
					var r = confirm( "<?php esc_html_e( 'Warning: This action will reset all current customizations.', 'the-affair' ); ?>" );
						...
						try {
							...
							if( typeof info.success != 'undefined' && info.success == true ){
								...
								if( typeof info.data != 'undefined' ){
									...
									alert( '<?php esc_html_e( 'Error server!', 'the-affair' ); ?>' );
								}
							}
						} catch (e) {
							alert( response );
